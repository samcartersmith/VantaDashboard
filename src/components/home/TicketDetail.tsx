...
export function TicketDetail({
  ...
  const goBack = () => (backKey ? openMetric(backKey) : closeModal());
      ...
      <div className="flex items-start justify-between border-b border-stone-100 px-5 py-4">
        <div className="flex flex-wrap items-center gap-1.5">
          {backKey && (
            <button
              onClick={goBack}
              className="mr-1 inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[12px] text-stone-500 hover:bg-stone-100"
            >
        ...
        {item.snooze && (
          <div className="mt-4 rounded-lg border border-amber-100 bg-amber-50 px-3 py-2 text-[12px] text-amber-800">
            <span className="font-medium">
              Snoozed until {shortDate(item.snooze.until)}
            </span>
      ...
      {!resolved && (
        <div className="flex items-center gap-1.5 border-t border-stone-100 px-5 py-3">
          {item.status !== "IN_PROGRESS" && (
            <Tooltip label="Mark as in progress">
              <button
          ...
          <div className="relative" ref={reassignAnchor}>
            <Tooltip label="Assign to a person or team">
              <button
          ...
          <Tooltip label="Defer with an audit note">
            <button
          ...
          <Tooltip label="Dismiss this item" side="top">
            <button
          ...
          <Tooltip label="Resolve — remediation verified">
            <button
              onClick={() => resolveItem(id)}
